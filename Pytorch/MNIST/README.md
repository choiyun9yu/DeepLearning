...
#### Labeling 작업에 대한 사전 고려사항
- Labeling 영역의 지정에 대한 규칙 필요
    - 인식하고자 하는 대상과의 거리
    - 시선의 초점으로부터 허용되는 상하좌우 범위(각도)
    - 인식 대상에 대한 선명도(흐릿함)의 허용 범위
    - 인색 대상에 적용되는 밝기의 허용 범위
    - 인식 대상의 크기는 다양하게 적용 가능하도록 데이터셋 구성
- 영상 전처리 범위에 대한 규칙 필요
    - 입력 영상의 주변 환경을 기준으로 입력 영상에 대한 표준 밝기
...
- 레이블링 작업자의 훈련과 퍼포먼스 측정 등을 포함하는 인력 관리
- 레이블링 과정 중 이슈 발생시 커뮤니케이션 비용 관리
- 레벨링을 통해 구축된 데이터셋의 버전관리 등

#### Labeling 작업의 중요성
- 이미지, 영상 데이터의 레이블링 작업은 가장 덜 중요한 것으로 인식
- 그러나 컴퓨터 비전 문제를 푸는 데 있어서 가장 선도적인 테슬라 팀이 여전히 난항을 겪고 있는 부분이 라벨링 워크플로우라고 지적
- 상용화 단계에서 컴퓨터 비전 문제를 해결하기 위해 우리가 주목해야할 부분은, 사실은 데이터였다.
